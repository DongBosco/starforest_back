...
public interface ReservationRepository extends JpaRepository<ReservationDates, Long>{
	 ...
	 @Query("SELECT r FROM ReservationDates r WHERE r.campSite.id = :id AND (r.start_date < :endDate AND r.end_date > :startDate)")
	...
	@Query("SELECT r FROM ReservationDates r WHERE r.campSite.id = :id")
